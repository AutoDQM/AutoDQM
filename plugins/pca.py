...
def predict(hist, pca):
    ...
    if False: #(n_dim != 1) or (n_dim !=2): # check for 1D or 2D hists
        raise ValueError()
    
    orig_shape = hist.shape
    ...
    sse = numpy.sum(
        (hist.reshape(1,-1) - hist_reconstructed) ** 2
    ).flatten()

    return float(sse), hist_reconstructed.reshape(orig_shape)
...
def plot(histpair, hist, hist_reconstructed):
    ...
    ## check then plot 1D or 2D 
    c = go.Figure()
    if len(hist_reconstructed.shape)==1:
...
def pca(histpair, **kwargs):
    ...
    f_pca = "/var/www/cgi-bin/models/pca/{0}/{1}/{2}.json".format(histpair.config["jar_dir"], year, histpair.data_name)
    #f_pca = "/root/AutoDQM/models/pca/{0}/{1}/{2}.json".format(histpair.config["jar_dir"], year, histpair.data_name)
    ...
    if not ( (len(hist.shape) ==1) or (len(hist.shape)==2)): # only for 1d or 2d histograms
        show = False
        return None

    if numpy.prod(hist.shape) != pca.n_features_: # number of bins don't match (can happen if you try to apply a PCA trained on histograms from one year on another year, where that histogram has the same naming but its binning has been changed)
        # np.prod is needed for case of 2d histograms. n_feature will equal product of the shape
        show = False
        return None
