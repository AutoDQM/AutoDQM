docker ps > temp.txt
if [[ "${1}" == "open" ]] ; then
    python tools.py open
elif [[ "${1}" == "logs" ]] ; then
    python tools.py logs
elif [[ "${1}" == "scp" ]] ; then
    if [[ "${2}" != "" ]] && [[ "${3}" != "" ]] ; then
        python tools.py scp ${2} ${3}
    else
    ...
    python tools.py scp cern-cert.public.pem /run/secrets/cmsvo-cert.pem
    python tools.py scp cern-cert.private.key /run/secrets/cmsvo-cert.key
elif [[ "${1}" == "stop" ]] ; then
    docker stack rm autodqm
    python tools.py "stop"
else
